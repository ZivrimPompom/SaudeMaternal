...
-- 3. Create the patients table (Pacientes) - Essential for Maternal Health
...
-- 5. Create basic policies (Allow all for development, should be hardened for production)
DROP POLICY IF EXISTS "Allow all access for development" ON public.unidades_saude;
...
-- 8. Create the professional categories table (Categorias Profissionais)
...
-- 13. Enable Row Level Security (RLS) for the new table
ALTER TABLE public.profissionais ENABLE ROW LEVEL SECURITY;

-- 14. Create basic policies for the new table
CREATE POLICY "Allow all access for development" ON public.profissionais
    FOR ALL
    USING (true)
    WITH CHECK (true);

-- 15. Trigger for updated_at for the new table
CREATE TRIGGER update_profissionais_updated_at
    BEFORE UPDATE ON public.profissionais
    FOR EACH ROW
    EXECUTE PROCEDURE update_updated_at_column();

-- 16. Create the routines table (Rotinas)
...
-- 21. Enable Row Level Security (RLS) for the new table
ALTER TABLE public.gestacoes ENABLE ROW LEVEL SECURITY;

-- 22. Create basic policies for the new table
...
-- 23. Trigger for updated_at for the new table
...
-- 24. Create the appointments table (Atendimentos)
...
-- 25. Enable Row Level Security (RLS) for the new table
ALTER TABLE public.atendimentos ENABLE ROW LEVEL SECURITY;

-- 26. Create basic policies for the new table
CREATE POLICY "Allow all access for development" ON public.atendimentos
    FOR ALL
    USING (true)
    WITH CHECK (true);

-- 27. Trigger for updated_at for the new table
